...
typedef
_(dryad "sll:srtl:sll_R:srtl_R:keys:llen_next:lseg:slseg:lseg_R:slseg_R:lseg_keys:lseg_len_next")
struct s_node {
  int key;
  struct s_node * next;
} Node;
  ...
  _(ensures (srtl(tl) && \oset_disjoint(srtl_reach(tl), srtl_lseg_reach(hd, tl))) ==>
             (srtl(hd) 
              && srtl_reach(hd) == \oset_union(srtl_lseg_reach(hd, tl), srtl_reach(tl))
						  ...
						  //&& sll_mkeys(hd) == \intbag_union(sll_mkeys(tl), sll_lseg_mkeys(hd, tl))
              && sll_list_len_next(hd) == (sll_lseg_len_next(hd, tl) + sll_list_len_next(tl)) ) ) 
  _(ensures (hd != NULL && tl != NULL && srtl(tl->next) && srtl_lseg(hd, tl)
         ...
         ( \oset_in(tl, srtl_reach(hd))  
           &&   srtl_lseg (hd, tl->next) 
				   && sll_lseg_keys(hd, tl->next) == \intset_union(\intset_singleton(tl->key), sll_lseg_keys(hd, tl)) 
				   //&& sll_lseg_mkeys(hd, tl->next) == \intbag_union(\intbag_singleton(tl->key), sll_lseg_mkeys(hd, tl))
				   && srtl_lseg_reach(hd, tl->next) == \oset_union_one1(tl, srtl_lseg_reach(hd, tl))   
           && sll_lseg_len_next(hd, tl->next) == (sll_lseg_len_next(hd, tl) + 1) ) )
  ;)
...
_(abstract _(dryad "base:slseg_R") \oset srtl_lseg_reach(struct s_node * hd, struct s_node * tl)
	...
	_(ensures hd == tl ==> \result == \oset_empty())
	_(ensures (hd != NULL && hd != tl) ==> \oset_in(hd, \result))
	_(ensures (hd != NULL && tl == NULL) ==> (\result == srtl_reach(hd)))
  ;)
...
_(logic _(dryad "unfold:srtl_R") \bool unfold_srtl_reach(struct s_node * hd) =
	(hd != NULL ==>
		(srtl_reach(hd) == \oset_union_one2(srtl_reach(hd->next), hd)) ) 
;)
...
_(logic _(dryad "unfold:lseg_R") \bool unfold_sll_lseg_reach(struct s_node * hd, struct s_node * tl) =
	((hd != NULL && hd != tl) ==> (sll_lseg_reach(hd, tl) == \oset_union_one2(sll_lseg_reach(hd->next, tl), hd)) ) ;)
_(logic _(dryad "unfold:slseg_R") \bool unfold_srtl_lseg_reach(struct s_node * hd, struct s_node * tl) =
	((hd != NULL && hd != tl) ==> (srtl_lseg_reach(hd, tl) == \oset_union_one2(srtl_lseg_reach(hd->next, tl), hd)));)

_(logic _(dryad "unfold:lseg_keys") \bool unfold_sll_lseg_keys(struct s_node * hd, struct s_node * tl) =
 ((hd != NULL && hd != tl) ==> sll_lseg_keys(hd, tl) == \intset_union(sll_lseg_keys(hd->next, tl), \intset_singleton(hd->key)) ) ;)
...
;)
_(logic \bool same_srtl_all_un(struct s_node * hd, struct s_node * tl, \state enter, \state exit) = 
...
//_(logic _(dryad "disj:mkeys") 
//  \bool disj_same_sll_mkeys(\oset heaplet, struct s_node * y, \state enter, \state exit) =
//	  (\oset_disjoint(\at(enter, heaplet), \at(enter, sll_reach(y)))) ==> \at(enter, sll_mkeys(y)) == \at(exit, sll_mkeys(y)) ;)
...
//  	&& disj_same_sll_mkeys(\at(enter, heaplet), y, enter, exit) 
...
_(logic _(dryad "disj:lseg_keys")
  \bool disj_same_sll_lseg_keys(\oset heaplet, struct s_node * hd, struct s_node * tl, 
    ...
    ((\oset_disjoint(\at(enter, heaplet), \at(enter, sll_lseg_reach(hd, tl)))) ==>
      \at(enter, sll_lseg_keys(hd, tl)) == \at(exit, sll_lseg_keys(hd, tl))) ;)
/*_(logic _(dryad "disj:lseg_mkeys")
  \bool disj_same_sll_lseg_mkeys(\oset heaplet, struct s_node * hd, struct s_node * tl, 
                            \state enter, \state exit) =
    ((\oset_disjoint(\at(enter, heaplet), \at(enter, sll_lseg_reach(hd, tl)))) ==>
      \at(enter, sll_lseg_mkeys(hd, tl)) == \at(exit, sll_lseg_mkeys(hd, tl))) ;) */
...
_(logic 
  \bool disj_same_sll_all_bin(\oset heaplet, struct s_node * hd, struct s_node * tl, \state enter, \state exit) =
  	...
  	//&& disj_same_sll_lseg_mkeys(\at(enter, heaplet), hd, tl, enter, exit)
...
//         && (sll_list_len_next(b) == (sll_lseg_len_next(b, e) + sll_list_len_next(e)))))
  ...
  {
    if (b == NULL || b == e) { 
      //_(assume (sll(b) && sll(e) && sll_lseg(b, e)) ==> 
      //      ((sll_reach(b) == sll_lseg_reach(b, e) \union sll_reach(e))
      //   && (sll_list_len_next(b) == (sll_lseg_len_next(b, e) + sll_list_len_next(e)))))
      return;
    }else  {
...
//         && (sll_list_len_next(b) == (sll_lseg_len_next(b, e) + sll_list_len_next(e))))
  ...
  {
    if (b == NULL || b == e) { 
//    _(assume (srtl(b) && srtl(e) && srtl_lseg(b, e)) ==> 
//           (srtl_reach(b) == srtl_lseg_reach(b, e) \union srtl_reach(e))
//         && (sll_list_len_next(b) == (sll_lseg_len_next(b, e) + sll_list_len_next(e))))
      return;
    }else  {
