...
/*
    ...
    disjoint(reach(x),   reach(y))
    disjoint(reach(x,y), reach(y))
    disjoint(reach(x,y), reach(z))

    disjoint(singleton(x), reach(y))
    disjoint(singleton(x), reach(y,z))
...
/*
    ...
    keys(x,y) == keys(z,w)
        ...
        keys(x)  == union(    keys(y),  keys(z))
    old(keys(x)) == union(    keys(x),  keys(y))
    old(keys(x)) == union(    keys(z),  keys(y))
        keys(x)  == union(old(keys(x)), keys(y))
        keys(z)  == union(old(keys(x)), keys(y))
...
/*
  ...
  //len(x) - len(y) == i - j
    ...
    keys(x)   <= singleton(i)      <= keys(x)
    keys(x,y) <= singleton(i)      <= keys(x,y)

    keys(y)   <= singleton(x->key) <= keys(y)
    keys(y,x) <= singleton(x->key) <= keys(x,y)
    keys(y,z) <= singleton(x->key) <= keys(y,z)
*/
...
var patterns_data = [
  ...
  //['implies', ['and', ['lseg', 0, 1], ['list', 1], ['neq', 0, 1], ['non_null', 1]], ['le', ['max', 0, 1], ['min', 1]]],
  ...
  //['le_i', ['keys', 1, 0], ['singleton_i', ['key', 0]]], ['le_i', ['singleton_i', ['key', 0]], ['keys', 0, 1]], 
  //['le_i', ['keys', 1, 2], ['singleton_i', ['key', 0]]], ['le_i', ['singleton_i', ['key', 0]], ['keys', 1, 2]], 
];
