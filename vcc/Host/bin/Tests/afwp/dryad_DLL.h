...
_(abstract _(dryad "base:dll_lseg") \bool dll_lseg(struct d_node * hd, struct d_node * tl)
  ...
  _(ensures (hd != NULL && tl == NULL) ==> (\result == dll(hd)))
...
_(logic
  \bool disj_same_dll_alln_un(\oset heaplet, struct d_node * y, \state enter, \state exit) =
...
_(logic
  \bool disj_same_dll_alln_bin(\oset heaplet, struct d_node * hd, struct d_node * tl, \state enter, \state exit) =
...
_(logic \bool mutable_list(struct d_node * x) =  ((x != NULL) ==> (\mutable(x) && \writable(x))))
